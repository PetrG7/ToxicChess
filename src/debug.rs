...
use crate::pieces::{ PieceType, Piece, Colour };
...
pub fn draw(state: &BoardState) {
    ...
    for y in (1..=8).rev() {
        ...
        for x in 65..=72 { // - 8 + 1
        	...
        	let is_piece = current.iter().find(|piece| piece.get_x() == x && piece.get_y() == y);
			match is_piece{
				Some(piece) => {let unicode = piecetype_to_unicode(piece);
